1. What is the problem you are solving?
> Businesses or your audience care about problems. Make the problem clear and relatable

In todays world, most organisations 
measure employee performance but few 
understand what drives it. It is 
imperative that companies understand 
this as they lose money and talent when 
they can't identify what drives 
performance. When the gap between low and 
high performers is left unaddressed, it 
shows up in dissatisfied employees, 
reduced productivity etc. 

Using the Employee Performance and 
Productivity Dataset, this dashboard uses 
employee data to profile both ends of the 
performance spectrum to answer:

> What separates a top performer from a low performer and what does it cost a company when that gap goes unaddressed?



2. What tools did you use?
> Python? GeoPanadas? Google Big Query?
> Tell your audience how you got the answer,  tools show your capability
> Dive deep in this in your technical  article

Excel for cleaning (nulls, data types, calculated tiers) 
Tableau for visual analysis and the final dashboard.



3. What insights did you or do you want to discover?/ What Solutions do you want to offer? Do People even need these solutions?
> Don’t just say “dashboard done.”
> Share the “aha!” moments, they matter more that the visuals

1. Is there a gap between low and top performers? (KPI cards)
State of Company?
AVG Performance Score + Attrition Rate + Satisfaction

What's the driving force?
Top Performer % + Training Hours + Promotions

Question: Where is the gap worst?


2. Does the gap have an address? (Bar Chart)
Which departments have the highest 
concentration of low performers?

Departments & Performance Score & Employee ID

Question: What do people in the low-scoring dept lack?


3. Are low performers undertrained (Scatter Plot Chart)
Is there a link between training hours 
and performance score?

Training hours & Performance Score & Department

Question: Is it just training or are they unhappy?


4. Is it dissatisfaction? (Grouped bar Chart)
Do high performers have higher job 
satisfaction than low performers or does it vary?

Performance Score & Employee Satisfaction Score
Performance score tier:
low
mid
high

Question: Does where or how they work play a role?


5. What does their work setup look like? (Chart)
Do remote workers perform differently 
from on-site workers?

Perfomance score & Remote_Work_Frequency & Tenure 
Remote: 100
Hybrid: 25/50/75
On-site: 0

Question: So what actually happens to low performers?


6. What is the cost of the gap? (Chart)
Do low performers resign more or do high performers leave due to poor satisfaction??

Attrition rate & Performance tier & Tenure 



4. How would a business or a community (for Social Impact Projects) benefit from your work?
> Be specific. Think money saved, process improves, better decisions made
> That’s your **real value**

HR can spot at-risk employees before 
attrition spikes, managers can prioritize 
training budgets, and leadership can see 
which departments need structural support, 
not just motivation.